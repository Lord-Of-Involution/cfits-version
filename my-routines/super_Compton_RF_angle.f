!-------------------------------------------------------------------------------------------------      
      subroutine super_Compton_RF_angle(itrans, theta, nmaxp, wp, df,
     & skn,  mgi, smit, agt) 
      use probmod
...
c
      ...
      double precision :: cos1,cos2,cos3,cos4,x
      double precision :: p1(nmaxp),p2(nmaxp),p3(nmaxp),p4(nmaxp)
      integer ii, jj, kk, point(nmaxp), indices(nmaxp,nmaxp)
      integer :: n 
...
c$$$  double precision, allocatable :: srf_arr(:)
      ...
      double precision :: skkk,ccc,smel
c Initialize status
      status=0
c
...
!save the SRF array in order to pass it to the fits file routine   
            srf_arr(kk) = srf
            p1(kk)=profil(1,wp(kk),wp(jj),cos1,x)
            p2(kk)=profil(1,wp(kk),wp(jj),cos2,x)
            p3(kk)=profil(1,wp(kk),wp(jj),cos3,x)
            p4(kk)=profil(1,wp(kk),wp(jj),cos4,x)

         enddo
            call add_row_HDU(n, nmaxp, point(jj), indices(jj,1),
     &        skn(jj,iz), srf_arr,p1,p2,p3,p4, unit)
